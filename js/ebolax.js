        ...
        success: function (response) {
            ...
            for (var a in config.columns) {
                if (!config.columntypes.includes(config.columns[a].type)) {
                    alert(
                        "Column Type Error! Error In Column ID: " +
                            config.columns[a].id
                    );
                    return;
                }
...
function setup() {
    ...
    for (var a in config.columns) {
        ...
        form_fields +=
            '<label for="' +
            config.columns[a].id +
            '"><h6 class="mb-0 text-info"><i class="fa ' +
            config.columns[a].icon +
            ' mr-1"></i> ' +
            config.columns[a].name +
            " (" +
            config.columns[a].id +
            ")</h6>" +
            (config.columns[a].description
        ...
        switch (config.columns[a].type) {
            ...
            case "time":
                form_fields +=
                    '<input type="' +
                    (config.columns[a].isEmail ? "email" : "text") +
                    '" class="form-control mb-3 focus" id="' +
                    config.columns[a].id +
                    '" name="' +
                    config.columns[a].id +
                    '" value="' +
                    config.columns[a].default_value +
                    '" ' +
                    (config.columns[a].required ? "required" : "") +
                    " " +
                    (config.columns[a].maxlength > 0
                        ? 'maxlength="' + config.columns[a].maxlength + '"'
                        : "") +
                    " />";
            ...
            case "number":
                form_fields +=
                    '<input type="text" class="form-control mb-3 focus" id="' +
                    config.columns[a].id +
                    '" name="' +
                    config.columns[a].id +
                    '" value="' +
            ...
            case "textarea":
                form_fields +=
                    '<textarea class="form-control mb-3 focus" id="' +
                    config.columns[a].id +
                    '" name="' +
                    config.columns[a].id +
                    '" rows="3" ' +
            ...
            case "select":
                form_fields +=
                    '<select class="form-control mb-3 focus" id="' +
                    config.columns[a].id +
                    '" name="' +
                    config.columns[a].id +
                    '" ' +
            ...
            case "file":
                ...
                form_fields +=
                    ...
                    '"></label>';
                form_fields +=
                    '    <input type="file" class="custom-file-input focus" id="' +
                    config.columns[a].id +
                    '" name="' +
                    config.columns[a].id +
                    '" value="' +
                ...
                form_fields += "</div>";
                form_fields +=
                    '<div class="text-center mt-2"><a id="' +
                    config.columns[a].id +
                    '_del" name="' +
                    config.columns[a].id +
                    '_del" data-id="' +
                    config.columns[a].id +
                    '" class="btn btn-danger text-white btn-clear">Clear</a></div>';
            ...
            case "image":
                form_fields +=
                    ...
                    '_f" value="" />';
                form_fields +=
                    '<p class="mb-2 text-center"><a id="' +
                    config.columns[a].id +
                    '_link" href=""><img class="image img-fluid" id="' +
                    config.columns[a].id +
                    '_image" src="" /></a></p>';
                form_fields += '<div class="custom-file">';
                form_fields +=
    ...
    $(".btn-clear").on("click", function () {
        ...
        var type = "file";
        for (var a in config.columns) {
            if (column_id == config.columns[a].id)
                type = config.columns[a].type;
        }

        $("#form #" + column_id)
            .siblings(".custom-file-label")
            .html("");
        if (type == "file" || type == "sound" || type == "video")
            $("#form #" + column_id + "_link").text("");
    ...
    for (var a in config.columns) {
        if (
            config.columns[a].type == "textarea" &&
            config.columns[a].use_editor
        ) {
            $("#" + config.columns[a].id).tinymce({
                ...
                toolbar:
                    "code | bold italic forecolor backcolor | alignleft aligncenter alignright alignjustify",
            });
        }
    }
        ...
        if (confirm("Are you sure you want to remove row id " + id + "?")) {
            $.ajax({
                ...
                success: function (response) {
                    ...
                    alertMessage(
                        "Row Successfully Deleted - id: " + response.id,
                        "success"
                    );
    ...
    $(document).on("click", ".update", function () {
        ...
        for (var a in config.columns) {
            switch (config.columns[a].type) {
                ...
                case "date":
                case "time":
                    $("#form #" + config.columns[a].id).val(
                        form_data[config.columns[a].id]
                    );

                    break;

                case "textarea":
                    $("#form #" + config.columns[a].id).text(
                        form_data[config.columns[a].id]
                    );
                ...
                case "file":
                    $("#form #" + config.columns[a].id)
                        .siblings(".custom-file-label")
                        .html("");
                    $("#form #" + config.columns[a].id + "_link")
                        .text(form_data[config.columns[a].id])
                        .attr(
                            "href",
                            config.uploads_path +
                                form_data[config.columns[a].id]
                ...
                case "image":
                    $("#form #" + config.columns[a].id)
                        .siblings(".custom-file-label")
                        .html("");
                    $("#form #" + config.columns[a].id + "_link").attr(
                        "href",
                    ...
                    $("#form #" + config.columns[a].id + "_image").attr(
                        "src",
                        form_data[config.columns[a].id] != ""
                            ? config.uploads_path +
                                  form_data[config.columns[a].id]
                ...
                case "sound":
                    ...
                    $("#form #" + config.columns[a].id + "_sound").attr(
                        "src",
                        form_data[config.columns[a].id] != ""
                            ? config.uploads_path +
                                  form_data[config.columns[a].id]
                ...
                case "video":
                    ...
                    $("#form #" + config.columns[a].id + "_video").attr(
                        "src",
                        form_data[config.columns[a].id] != ""
                            ? config.uploads_path +
                                  form_data[config.columns[a].id]
...
function fetch_data() {
    ...
    for (var a = 1; a <= config.columns.length; a++) {
        ...
        if (
            config.columns[a - 1].type == "textarea" ||
            ...
            config.columns[a - 1].type == "video"
        )
            control = true;

        if (!config.columns[a - 1].searchable || control)
            searchable_false.push(a);
    ...
    for (var a in config.columns) {
        columnDefs.push({
            width: config.columns[a].width ? config.columns[a].width : "100px",
            targets: [parseInt(a) + 1],
        });
    }
